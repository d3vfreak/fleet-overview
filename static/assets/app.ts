...
function showHelp() {
  ...
  vex.dialog.alert({
    ...
    This application checks every 30 seconds if you are in a fleet so you maybe need wait a bit more for it to see that you are in fleet.<br/><br/>CCP is updating the fleet data every 5 seconds and this application get the data every 6 seconds.<br/><br/> The function that aggregates the data can take some additional seconds so it can take up to 14 seconds to update the results.`,
  });
}
